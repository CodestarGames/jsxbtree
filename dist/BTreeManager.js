...
export class BTreeManager {
    ...
    start(Tree, tick = -1, blackboard, debugTarget) {
    ...
    *processTick(tree) {
     ...
     */
    _applyLeafNodeGuardPaths(tree) {
        this._getAllNodePaths(tree).forEach((path) => {
            ...
            for (let depth = 0; depth < path.length; depth++) {
                ...
                currentNode.guardPath = new GuardPath(path
                    .slice(0, depth + 1)
    ...
    _getAllNodePaths(tree) {
        ...
        const findLeafNodes = (path, node) => {
            ...
            if (node.isLeafNode()) {
                ...
                node.children.forEach((child) => findLeafNodes(path, child));
            }
        };
    ...
    ;
    renderDebugger(container, targetElem) {
        if (!targetElem) {
                    ...
                    default: {
                        ...
                        template: (node) => {
                            if (node.item.decorators) {
                                const getDecoratorHTMl = () => node.item.decorators.map((decorator) => {
...
//# sourceMappingURL=BTreeManager.js.map